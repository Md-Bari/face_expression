...
for the 5 target emotion classes: Angry, Fear, Happy, Sad, Suprise.
"""
...
# 5 Emotion Class Names (matching user dataset folders)
...
# Color palette for the 5 emotions (BGR for OpenCV, Hex for UI)
EMOTION_PALETTE = {
...
class FaceEngine:
    """Manages face detection and 5-class emotion classification pipelines."""
    ...
    def get_available_models(self):
        ...
        for item in os.listdir(model_dir):
            if item.endswith(('.onnx', '.keras', '.tflite')) and not item.startswith('face_detection'):
                ...
                model_type = "ONNX 5-Class Model (48x48 RGB)" if "face_emotion" in item else \
                             "ONNX Neural Network" if "onnx" in item else \
                             "Keras Model" if "keras" in item else \
                             "TFLite Edge Model"
    ...
    def detect_faces(self, bgr_image):
        ...
        # Fallback to whole frame / center region
        if not faces:
            min_dim = min(h, w)
            cw, ch = int(min_dim * 0.8), int(min_dim * 0.8)
            cx, cy = (w - cw) // 2, (h - ch) // 2
            faces.append({
                'box': [cx, cy, cw, ch],
                'confidence': 0.85,
    ...
    def preprocess_face(self, bgr_image, box, padding=0.15):
        """
        Crops face with padding and converts to 48x48 RGB NCHW format for face_emotion_model.onnx.
        """
        ...
        # Normalized float32 tensor [0.0, 1.0]
        tensor = face_rgb.astype(np.float32) / 255.0
        # (48, 48, 3) -> (3, 48, 48) -> (1, 3, 48, 48)
    ...
    def predict_expression(self, face_tensor, model_name='face_emotion_model.onnx'):
        """
        Predicts 5-class emotion probabilities using face_emotion_model.onnx.
        Target classes: ['Angry', 'Fear', 'Happy', 'Sad', 'Suprise']
        """
        target_key = 'face_emotion_model.onnx'
        ...
        if target_key in self.onnx_sessions:
            try:
                ...
                raw_out = outputs[0][0]  # Raw logits

                # Map directly to the 5 target classes: ['Angry', 'Fear', 'Happy', 'Sad', 'Suprise']
    ...
    def process_frame(self, image_data, model_name='face_emotion_model.onnx', draw_annotations=True):
        ...
        Returns detection summary, faces, 5-class probabilities, and base64 annotated image.
        """
        ...
        for idx, face_info in enumerate(faces_detected):
            box = face_info['box']
            face_crop, face_tensor = self.preprocess_face(bgr_image, box)
            ...
            if draw_annotations:
                ...
                cv2.line(annotated_bgr, (x, y + h), (x + corner_len, y + h), (255, 255, 255), 3)
                cv2.line(annotated_bgr, (x, y + h), (x, y + h - corner_len), (255, 255, 255), 3)
                cv2.line(annotated_bgr, (x + w, y + h), (x + w - corner_len, y + h), (255, 255, 255), 3)
                cv2.line(annotated_bgr, (x + w, y + h), (x + w, y + corner_len), (255, 255, 255), 3)
        ...
        primary = results[0] if results else {
            ...
            'confidence': 0.0,
            'probabilities': {k: 20.0 for k in CLASS_NAMES},
            'emoji': '😐',
            'color': '#94a3b8'
        }
